...
export default function PreparePage() {
  ...
  const buildMap = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/subtopic-map', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ topic, language }) });
      ...
      const dres = await fetch('/api/diagnostic-quiz', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ topic, subtopics: sample, language }) });
  ...
  const startDrill = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/drill-quiz', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ topic, targetSubtopics: weakSubtopics, targetDifficulty: drillDifficulty, language }) });
      if (!res.ok) throw new Error(await res.text());
  ...
  }, [topic, weakSubtopics, drillDifficulty, language]);
